...
class CampaignsRewardsApi(val defBasePath: String = "https://integration.knetikcloud.com/",
   ...
   */
  def getRewardSetsUsingGET(size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/, order: Option[String] /* = id:ASC*/): Option[PageResourceRewardSetResource] = {
    // create path and map variables
    val path = "/rewards".replaceAll("\\{format\\}", "json")
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(apiInvoker.deserialize(s, "", classOf[PageResourceRewardSetResource]).asInstanceOf[PageResourceRewardSetResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
