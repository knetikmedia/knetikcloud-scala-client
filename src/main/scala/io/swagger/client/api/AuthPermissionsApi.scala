...
class AuthPermissionsApi(val defBasePath: String = "https://localhost:8080/",
   ...
   */
  def getPermissionUsingGET1(size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/, order: Option[String] /* = permission:ASC*/): Option[PagePermissionResource] = {
    // create path and map variables
    val path = "/auth/permissions".replaceAll("\\{format\\}", "json")
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[PagePermissionResource]).asInstanceOf[PagePermissionResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
