...
class GamificationLevelingApi(val defBasePath: String = "https://localhost:8080/",
   ...
   */
  def getLevelsUsingGET(filterName: Option[String] = None, size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/, order: Option[String] /* = name:ASC*/): Option[PageLevelingResource] = {
    // create path and map variables
    val path = "/leveling".replaceAll("\\{format\\}", "json")
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[PageLevelingResource]).asInstanceOf[PageLevelingResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
   ...
   */
  def getUserLevelsUsingGET(userId: Integer): Option[PageUserLevelingResource] = {
    // create path and map variables
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[PageUserLevelingResource]).asInstanceOf[PageUserLevelingResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
