...
class MediaModerationApi(val defBasePath: String = "https://localhost:8080/",
   ...
   * @return void
   */
  def getFlagReportUsingGET(id: Long) = {
   ...
   */
  def getFlagsReportUsingGET(excludeResolved: Option[Boolean] /* = true*/, filterContext: Option[String] = None, size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/): Option[PageFlagReportResource] = {
    // create path and map variables
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[PageFlagReportResource]).asInstanceOf[PageFlagReportResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
   ...
   * @return void
   */
  def setFlagResolutionUsingPUT(id: Long, flagReportResource: Option[FlagReportResource] = None) = {
    // create path and map variables
    val path = "/moderation/reports/{id}".replaceAll("\\{format\\}", "json").replaceAll("\\{" + "id" + "\\}",apiInvoker.escape(id))
