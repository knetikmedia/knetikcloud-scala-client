...
class BRERuleEngineTriggersApi(val defBasePath: String = "https://localhost:8080/",
   ...
   */
  def getTriggersUsingGET(filterSystem: Option[Boolean] = None, filterCategory: Option[String] = None, filterName: Option[String] = None, size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/): Option[PageBreTriggerResource] = {
    // create path and map variables
    val path = "/bre/triggers".replaceAll("\\{format\\}", "json")
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[PageBreTriggerResource]).asInstanceOf[PageBreTriggerResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
