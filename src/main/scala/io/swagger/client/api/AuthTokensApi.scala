...
class AuthTokensApi(val defBasePath: String = "https://localhost:8080/",
   ...
   * @return void
   */
  def deleteTokenUsingDELETE(username: String) = {
    ...
    if (username == null) throw new Exception("Missing required parameter 'username' when calling AuthTokensApi->deleteTokenUsingDELETE")
   ...
   * @return void
   */
  def deleteTokenWithClientIdUsingDELETE(username: String, clientId: String) = {
    // create path and map variables
    ...
    if (username == null) throw new Exception("Missing required parameter 'username' when calling AuthTokensApi->deleteTokenWithClientIdUsingDELETE")

    if (clientId == null) throw new Exception("Missing required parameter 'clientId' when calling AuthTokensApi->deleteTokenWithClientIdUsingDELETE")
   ...
   */
  def getTokenByUserUsingGET(username: String, clientId: String): Option[OauthAccessTokenResource] = {
    // create path and map variables
    val path = "/auth/tokens/{username}/{client_id}".replaceAll("\\{format\\}", "json").replaceAll("\\{" + "username" + "\\}",apiInvoker.escape(username)).replaceAll("\\{" + "client_id" + "\\}",apiInvoker.escape(clientId))
    ...
    if (username == null) throw new Exception("Missing required parameter 'username' when calling AuthTokensApi->getTokenByUserUsingGET")

    if (clientId == null) throw new Exception("Missing required parameter 'clientId' when calling AuthTokensApi->getTokenByUserUsingGET")
   ...
   */
  def getTokensUsingGET(filterClientId: Option[String] = None, filterUsername: Option[String] = None, size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/, order: Option[String] /* = username:ASC*/): Option[PageOauthAccessTokenResource] = {
    // create path and map variables
    ...
    try {
      apiInvoker.invokeApi(basePath, path, "GET", queryParams.toMap, formParams.toMap, postBody, headerParams.toMap, contentType) match {
        case s: String =>
           Some(ApiInvoker.deserialize(s, "", classOf[PageOauthAccessTokenResource]).asInstanceOf[PageOauthAccessTokenResource])
        case _ => None
      }
    } catch {
      case ex: ApiException if ex.code == 404 => None
