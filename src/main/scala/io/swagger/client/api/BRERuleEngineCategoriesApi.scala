...
class BRERuleEngineCategoriesApi(val defBasePath: String = "https://integration.knetikcloud.com/",
   ...
   * @return void
   */
  def updateBRECategory(name: String, category: Option[BreCategoryResource] = None) = {
    // create path and map variables
    val path = "/bre/categories/{name}".replaceAll("\\{format\\}", "json").replaceAll("\\{" + "name" + "\\}",apiInvoker.escape(name))
   ...
   * @return void
   */
  def updateBRECategoryTemplate(id: String, template: Option[TemplateResource] = None) = {
    // create path and map variables
    val path = "/bre/categories/templates/{id}".replaceAll("\\{format\\}", "json").replaceAll("\\{" + "id" + "\\}",apiInvoker.escape(id))
