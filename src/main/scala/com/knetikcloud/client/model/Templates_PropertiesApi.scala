...
class Templates_PropertiesApi(val defBasePath: String = "https://jsapi-integration.us-east-1.elasticbeanstalk.com",
  ...
  val helper = new Templates_PropertiesApiAsyncHelper(client, config)
...
class Templates_PropertiesApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getTemplatePropertyType(_type: String)(implicit reader: ClientResponseReader[PropertyFieldListResource]): Future[PropertyFieldListResource] = {
    ...
    if (_type == null) throw new Exception("Missing required parameter '_type' when calling Templates_PropertiesApi->getTemplatePropertyType")
