...
class Content_CommentsApi(val defBasePath: String = "https://jsapi-integration.us-east-1.elasticbeanstalk.com",
  ...
  val helper = new Content_CommentsApiAsyncHelper(client, config)
...
class Content_CommentsApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getComments(context: String,
    ...
    if (context == null) throw new Exception("Missing required parameter 'context' when calling Content_CommentsApi->getComments")
