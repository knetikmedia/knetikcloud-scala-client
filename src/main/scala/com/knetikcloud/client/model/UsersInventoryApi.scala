...
class UsersInventoryApi(val defBasePath: String = "https://sandbox.knetikcloud.com",
  ...
  */
  ...
  def getEntitlementItems(size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/, order: Option[String] /* = id:ASC*/): Option[PageResourceEntitlementItem] = {
    val await = Try(Await.result(getEntitlementItemsAsync(size, page, order), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
  ...
  */
  def getEntitlementItemsAsync(size: Option[Integer] /* = 25*/, page: Option[Integer] /* = 1*/, order: Option[String] /* = id:ASC*/): Future[PageResourceEntitlementItem] = {
      helper.getEntitlementItems(size, page, order)
  }
...
class UsersInventoryApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getEntitlementItems(size: Option[Integer] = Some(25),
