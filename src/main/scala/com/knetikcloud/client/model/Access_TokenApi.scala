...
class Access_TokenApi(val defBasePath: String = "https://jsapi-integration.us-east-1.elasticbeanstalk.com",
  ...
  val helper = new Access_TokenApiAsyncHelper(client, config)
...
class Access_TokenApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getOAuthToken(grantType: String = client_credentials,
    ...
    if (grantType == null) throw new Exception("Missing required parameter 'grantType' when calling Access_TokenApi->getOAuthToken")

    if (clientId == null) throw new Exception("Missing required parameter 'clientId' when calling Access_TokenApi->getOAuthToken")
