...
class Reporting_OrdersApi(val defBasePath: String = "https://jsapi-integration.us-east-1.elasticbeanstalk.com",
  ...
  val helper = new Reporting_OrdersApiAsyncHelper(client, config)
...
class Reporting_OrdersApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getInvoiceReports(currencyCode: String,
    ...
    if (currencyCode == null) throw new Exception("Missing required parameter 'currencyCode' when calling Reporting_OrdersApi->getInvoiceReports")
