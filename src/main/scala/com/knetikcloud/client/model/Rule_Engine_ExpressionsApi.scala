...
class Rule_Engine_ExpressionsApi(val defBasePath: String = "https://jsapi-integration.us-east-1.elasticbeanstalk.com",
  ...
  val helper = new Rule_Engine_ExpressionsApiAsyncHelper(client, config)
...
class Rule_Engine_ExpressionsApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getBREExpression(_type: String)(implicit reader: ClientResponseReader[ExpressionResource]): Future[ExpressionResource] = {
    ...
    if (_type == null) throw new Exception("Missing required parameter '_type' when calling Rule_Engine_ExpressionsApi->getBREExpression")
