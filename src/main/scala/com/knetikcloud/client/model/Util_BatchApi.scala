...
class Util_BatchApi(val defBasePath: String = "https://jsapi-integration.us-east-1.elasticbeanstalk.com",
  ...
  val helper = new Util_BatchApiAsyncHelper(client, config)
...
class Util_BatchApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def getBatch(token: String)(implicit reader: ClientResponseReader[List[BatchReturn]]): Future[List[BatchReturn]] = {
    ...
    if (token == null) throw new Exception("Missing required parameter 'token' when calling Util_BatchApi->getBatch")
